...
export const IncludeScripts = props => {
  ...
  if (!cookieConsent?.enabled || userHasAcceptedCookies) {
    ...
    if (facebookPixelId) {
      analyticsLibraries.push(
        <script key="facebook-pixel" async src={`${rootURL}/static/scripts/fb-pixel.js`}></script>
      );

      analyticsLibraries.push(
        <script key="facebook-pixel-init">{`fbq('init', '${facebookPixelId}'); fbq('track', 'PageView');`}</script>
      );
