...
const findValidImageUrl = (img) => {
  if (!img?.attributes) return null;
  ...
  for (const variant of possibleVariants) {
    const url = img?.attributes?.variants?.[variant]?.url;
    if (url) return url;
  }
...
const processListingImages = (images) => {
  if (!images || !Array.isArray(images)) {
    console.warn('⚠️ No images array provided');
    return [];
  }
  ...
  images.forEach((img, idx) => {
    ...
    console.log(`  Image ${idx + 1}:`, validUrl ? '✅ Found' : '❌ Not found', validUrl);
...
export const ListingCard = props => {
  ...
  // Get image entities from Redux store
  ...
  // === PROSES GAMBAR - FIXED: Get images from Redux store ===
  console.log('🖼️ ListingCard rendering for:', id);
  console.log('📸 currentListing.images:', currentListing.images);
  console.log('🔗 currentListing.relationships?.images:', currentListing.relationships?.images);
  console.log('🗂️ imageEntities from Redux:', imageEntities);
  
  // Get images from relationships and Redux store
  const imagesFromStore = getImagesFromStore(currentListing, imageEntities);
  console.log('📦 Images retrieved from store:', imagesFromStore);
  
  // Try currentListing.images first, fallback to store
  const imagesToProcess = currentListing.images?.length > 0 
    ? currentListing.images 
    : imagesFromStore;
  
  console.log('🎯 Final images to process:', imagesToProcess);
  ...
  const onToggleFavorites = e => {
    ...
    const customOnUpdate = payload => {
      if (isFavorite) {
        dispatch(unfavoriteListing(id)).catch(err => {
